...
✨ Key Features (Implemented & Planned)1. Interactive Hero SectionFull Viewport Video: Plays an intro sequence on load.Interactive State: User presses a button to continue the cinematic sequence.Mute Toggle: Custom UI control for audio experience.2. Navigation & UIResponsive Hamburger Menu: Custom animated icon (turns into 'X') with a blurred backdrop overlay.Profile Dummy: A placeholder login/profile dropdown for future backend integration.3. The FleetTop Picks Section: A CSS Grid layout showcasing the best cars.Hover Effects: Cards expand to show "Book Now" buttons and 0-60 stats.🚀 How to Run LocallyClone the repository:git clone [https://github.com/YOUR_USERNAME/Lumina-Motors.git](https://github.com/YOUR_USERNAME/Lumina-Motors.git)
Open the folder:cd Lumina-Motors
Launch:Simply open index.html in your browser.Recommended: Use the "Live Server" extension in VS Code for hot-reloading.📝 Future Roadmap[ ] Implement backend for "Profile" and "Booking" logic.[ ] Add "Our Commitment" section (Shuttle Service & Hotline banners).[ ] Optimize video assets for mobile data.🤝 CreditsDeveloper: [Your Name]3D Assets: Created in Blender by [Your Name]Built for the Web Development End-Term Project (Dec 2025).